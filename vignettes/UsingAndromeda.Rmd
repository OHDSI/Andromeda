...
`Andromeda` heavily relies on `arrow`, an R wrapper around the C++ Arrow library. Arrow provides platform and language independent tabular data representations. Arrow provides an interface to on-disk tabular data, called a `FileSystemDataset`, which can be manipulated with `dplyr`. An andromeda object is essentially a list of `FileSystemDataset`s.
...
`Andromeda` relies on `dplyr` for querying the data. A key aspect of `dplyr` is lazy execution. This means that we can define a query, but the query will not be executed until we explicitly request so using the `collect()` statement. For example, we may want to know the number of cars that can go faster than 10:
